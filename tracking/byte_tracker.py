...
class BYTETracker(object):
    ...
    def update(self, bboxes, scores, class_ids, img_info, img_size):
        ...
        if len(dets_keep) > 0:
            ...
            detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, c) for
                          (tlbr, s, c) in zip(dets_keep, scores_keep, class_ids_keep)]
        else:
            detections = []
    ...
    def predict(self, frame, bboxes, scores, class_ids):
        online_targets = self.update(bboxes, scores, class_ids, [frame.shape[0], frame.shape[1]], [frame.shape[0], frame.shape[1]])
        online_tlwhs = []
        online_ids = []
        online_scores = []
        online_class_ids = []
        ...
        return np.array(online_tlwhs), np.array(online_scores), np.array(online_class_ids), np.array(online_ids)
