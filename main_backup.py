...
yolov8_detector_silo = YOLOv8('weights/best.onnx', conf_thres=0.5, iou_thres=0.5)
...
def set_silo_selected_1(boxes, box_selected):
    global SILO_SELECTED_1
    
    SILO_SELECTED_1 = np.argmin(np.abs(np.sum(boxes - box_selected, -1))) + 1

def set_silo_selected_2(boxes, box_selected):
    global SILO_SELECTED_2
    
    SILO_SELECTED_2 = np.argmin(np.abs(np.sum(boxes - box_selected, -1))) + 1

def get_output(data, lenght=5):
    if STATE == STATE_DETECT_BALL:
        data = (lenght - len(data.split(';')))*'0;' + data +'\r'
    else:
        data += (lenght - len(data.split(';')))*';0' +'\r'
    return data
    ...
    while True:
        ...
        if STATE == STATE_DETECT_BALL:
            ...
            for i in ORDER_TO_GET_SILO:
                if i in freq.keys():
                    choiced_id.append(i)
                    if freq[i] > 1 or len(choiced_id) > 1:
                        break
            ...
            if len(boxes_filters) > 0:
                box_selecteds = []
                for boxes_filter in boxes_filters:
                    if len(boxes_filter) >= 2:
                        x_c, y_c = (boxes_filter[:,2] + boxes_filter[:,0])/2, (boxes_filter[:,3] + boxes_filter[:,1])/2
                        id_get = np.argsort(np.sqrt((x_c-CAM_X_CEN)**2 + (y_c-CAM_Y_CEN)**2))[:2]
                        box_selecteds.append(boxes_filter[id_get[0]])
                        box_selecteds.append(boxes_filter[id_get[1]])
                    else:
                        box_selected = boxes_filter[get_box_id(boxes_filter)]
                        box_selecteds.append(box_selected)
                
                # get 2 special boxes
                if len(box_selecteds) >= 2:
                    x, y = int(box_selecteds[0][2] + box_selecteds[0][0])//2, int(box_selecteds[0][3] + box_selecteds[0][1])//2
                    visualize_img = cv2.circle(visualize_img, (x, y), 13, (255, 255, 255) , -1)
                    subdata = f"{x};{y}"
                    x, y = int(box_selecteds[1][2] + box_selecteds[1][0])//2, int(box_selecteds[1][3] + box_selecteds[1][1])//2
                    visualize_img = cv2.circle(visualize_img, (x, y), 13, (255, 255, 255) , 3)
                else:
                    x, y = int(box_selecteds[0][2] + box_selecteds[0][0])//2, int(box_selecteds[0][3] + box_selecteds[0][1])//2
                    visualize_img = cv2.circle(visualize_img, (x, y), 13, (255, 255, 255) , -1)
                    subdata = f"{x};{y}"
                ...
                if len(boxes) >= 5:
                    set_silo_selected_1(boxes, box_selecteds[0])
                    if len(box_selecteds) >= 2:
                        set_silo_selected_2(boxes, box_selecteds[1])
            
                data = get_output(f'{len(boxes)};{SILO_SELECTED_1};{SILO_SELECTED_2};{subdata}')
            else:
                data = get_output(f'{len(boxes)};{SILO_SELECTED_1};{SILO_SELECTED_2}')
            print(data)
            ser.write(data.encode())
